...
public class Problem210Bad {
    ...
    public int[] findOrder(int numCourses, int[][] prerequisites) {
        ...
        for (Node node : idNodeMap.values()) {
            if (node.input == 0) {
                ...
                queue.addLast(node);
                addedToQueue.add(node.id);
                while (!queue.isEmpty()) {
                    Node h = queue.pollFirst();
                    if (marked.contains(h.id)) {
                        return new int[]{};
                    }
                    marked.add(h.id);
                    res.add(h.id);
                    for (Node nextClass : h.nextClass) {
                        if (!addedToQueue.contains(nextClass.id)) {
                            queue.addLast(nextClass);
                            addedToQueue.add(nextClass.id);
                        }
                    }
                }
            }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(new Problem210Bad().findOrder(num, courses)));
    }
}
